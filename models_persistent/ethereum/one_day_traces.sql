...
LEFT JOIN `bigquery-public-data.crypto_ethereum.transactions` AS tx
  ON tx.hash = tr.transaction_hash
WHERE tx.block_timestamp > current_timestamp() - interval {{ var('decode_source_interval') }}
AND tr.block_timestamp > current_timestamp() - interval {{ var('decode_source_interval') }}