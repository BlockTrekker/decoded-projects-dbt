...
def json_object_from_array(array_string, input_name, index_type, parent_path=None):
    ...
    if parent_path is None:
        parent_path = f"$.{index_type}.{input_name}"
    ...
    for section in sections:
        ...
        if nested_match:
            ...
            nested_query = json_object_from_array(nested_array, input_name, index_type, parent_path=new_parent_path)
            if nested_query:
                query_lines.append(f"'{name}', {nested_query[:-9]}")  # Remove the ", AS order" part from the nested query
        else:
            try:
                ...
                continue
            line = f"    '{name}', JSON_EXTRACT(decoded_values, '{parent_path}.{name}')"
...
WITH iface_extract AS (
  SELECT
    udfs.REDUCE_ABI_FOR_FUNCTION_NAME(abi, "{table_name.rsplit('_', 1)[-1]}") as  iface,
  FROM `blocktrekker.decoded_contracts.dune_abis`
    WHERE address IN ("0x1111111254eeb25477b68fb85ed929f73a960582")
...
cte AS (
    SELECT
        ...
        method_id,
        CASE 
            WHEN success = TRUE THEN udfs.DECODE_CALL_ENTRY(iface, input, output) 
            ELSE NULL 
...
WITH cte AS (
    SELECT
        ...
        udfs.DECODE_LOG_ENTRY(  0, topic1, topic2, topic3, data, abi) as decoded_values,
        evt_hash
    FROM {evt_table} AS l
    CROSS JOIN (
        SELECT
            ...
            # There's at least one input of type 'ARRAY' this will impact the query body
                for input_ in input_json_array:
                    if input_['type'].startswith('ARRAY'):
                        ...
                        json_object = json_object_from_array(input_['type'], input_['name'],input_['index_type'])
                        query_lines.append(json_object) 
                    else:
                        try:
                            query_lines.append(f"\n    JSON_EXTRACT_SCALAR(decoded_values, '$.{input_['index_type']}.{input_['name']}') AS `{input_['name']}`")
                        except KeyError as e:
                            query_lines.append(f"\n    JSON_EXTRACT_SCALAR(decoded_values, '$.{input_['index_type']}.{input_['name']}') AS `input_{input_count}`")
